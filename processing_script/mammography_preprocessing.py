...
CBIS_PNG_DIR = BASE_DIR / "CBIS-DDSM/png"
INBREAST_PNG_DIR = BASE_DIR / "INBreast"
PROCESSED_DIR = BASE_DIR / "processed_mammography"
...
for dataset in ['CBIS-DDSM', 'INBreast']:
    for split in ['train', 'test'] if dataset == 'CBIS-DDSM' else ['train', 'val']:
        for cls in CLASSES:
            os.makedirs(PROCESSED_DIR / dataset / split / cls, exist_ok=True)
...
def process_cbis_dataset():
    """Process CBIS-DDSM dataset"""
    print("📁 Processing CBIS-DDSM dataset...")
    metrics_list = []
    
    for split in ['train', 'test']:
        split_dir = CBIS_PNG_DIR / split
        if not split_dir.exists():
            print(f"⚠️ CBIS {split} directory not found: {split_dir}")
            continue
            
        for cls in CLASSES:
            class_dir = split_dir / cls
            processed_class_dir = PROCESSED_DIR / "CBIS-DDSM" / split / cls
            
            if not class_dir.exists():
                print(f"⚠️ CBIS {split}/{cls} directory not found: {class_dir}")
                continue
            ...
            print(f"📁 Processing CBIS-DDSM {split}/{cls} ({len(image_files)} images)...")
            
            with tqdm(total=len(image_files), desc=f"CBIS-{split}-{cls}", unit="img") as pbar:
...
def process_inbreast_dataset():
    """Process INBreast dataset"""
    print("📁 Processing INBreast dataset...")
    metrics_list = []
    
    if not INBREAST_PNG_DIR.exists():
        print(f"⚠️ INBreast PNG directory not found: {INBREAST_PNG_DIR}")
        return metrics_list
    
    for split in ['train', 'val']:
        split_dir = INBREAST_PNG_DIR / split
        if not split_dir.exists():
            print(f"⚠️ INBreast {split} directory not found: {split_dir}")
            continue
            
        for cls in CLASSES:
            class_dir = split_dir / cls
            processed_class_dir = PROCESSED_DIR / "INBreast" / split / cls
            
            if not class_dir.exists():
                print(f"⚠️ INBreast {split}/{cls} directory not found: {class_dir}")
                continue
            ...
            print(f"📁 Processing INBreast {split}/{cls} ({len(image_files)} images)...")
            
            with tqdm(total=len(image_files), desc=f"INBreast-{split}-{cls}", unit="img") as pbar:
                for img_file in image_files:
                    try:
                        # Read image
                        img = cv2.imread(str(img_file), cv2.IMREAD_GRAYSCALE)
...
def save_separate_metrics(all_metrics):
    ...
    comprehensive_filename = results_dir / f"mammography_preprocessing_metrics_{timestamp}.csv"
...
def main():
    ...
    if TEST_MODE:
        ...
        print("🚀 Starting full mammography preprocessing...")
    
    all_metrics = []
    
    # Process CBIS-DDSM
    cbis_metrics = process_cbis_dataset()
    all_metrics.extend(cbis_metrics)
    
    # Process INBreast
    inbreast_metrics = process_inbreast_dataset()
    all_metrics.extend(inbreast_metrics)
